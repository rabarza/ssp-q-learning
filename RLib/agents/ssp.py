...
class QAgent:
    ...
    def __init__(
        ...
        self.alpha_eval_expr = str(alpha).replace("N(s,a)", "N_sa").replace("N(s)", "N_s")
    ...
    def update_q_table(
        ...
        gamma = self.gamma
        bellman_target = reward + gamma * self.max_q_table(next_state)
        q_old = self.q_table[state][action]
        q_new = q_old * (1 - alpha) + alpha * (bellman_target)
    ...
    def save(self, path: str):
        ...
        save_path = os.path.join(path, f"{self.alpha_type}_alpha/{self.strategy}/")
...
class QAgentSSP(QAgent):
    ...
    def train(
        ...
        for episode in episodes_range:
            ...
            while not done:
                ...
                bellman_error = self.update_q_table(state, action, reward, next_state)
            ...
            # Verificar si el camino es óptimo
            ...
            # Actualizar los resultados del episodio
            self.update_results(
                episode,
                steps,
                total_score,
            ...
            message = f"Episodio {episode}/{num_episodes} - Pasos: {self.steps[episode]} - Max norm error: {normalized_error:.3f} - Max norm error SP: {normalized_error_shortest_path:.3f}"
            # Mostrar mensaje en streamlit o en consola
            episodes_range.set_description("Episodio {}/{}".format(episode, num_episodes))
    ...
    def results(self):
        ...
        results = {
            ...
            "max_norm_error": self.max_norm_error,
            "max_norm_error_shortest_path": self.max_norm_error_shortest_path,
            "max_norm_error_normalized": list(
                np.array(self.max_norm_error) / abs(optimal_cost)
            ),
            "max_norm_error_shortest_path_normalized": list(
                np.array(self.max_norm_error_shortest_path) / abs(optimal_cost)
            ),  # noqa: E501
            "optimal_cost": optimal_cost,
            "optimal_paths": self.optimal_paths,
        }
...
if __name__ == "__main__":
    ...
    graph = create_perceptron_graph(nodes_by_layer, 20, 2000, 20)
    ...
    q_agent.train(num_episodes=20000, shortest_path=shortest_path, q_star=optimal_q_table)
