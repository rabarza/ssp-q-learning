...
class EpsilonGreedyActionSelector(ActionSelector):
    def __init__(self, epsilon=0.1):
        ...
        """
        super().__init__(epsilon=0.1, strategy="ε-greedy")
...
class UCB1ActionSelector(ActionSelector):
    ...
    def select_action(self, agent, state):
        ...
        if not_chosen_actions_idx.shape[0] > 0:
            ...
            # Evitar divisiones por cero en el cálculo de UCB
            visits_state_action = np.where(
                visits_state_action == 0, 1e-6, visits_state_action
            )
