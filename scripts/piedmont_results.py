...
for root, dirs, files in os.walk(ruta_carpeta):
    # Identificar si la carpeta corresponde a un entorno "CityGraphs"
    if "CityGraphs" in root:
        ...
        if len(parts) < 3:
            continue
        graph_name = parts[-4:-2]  # Ejemplo: ['City_name', 'orig_node-dest_node']
        ...
        for strategy_dir in dirs:
            ...
            for agent_file in agent_files:
                try:
                    ...
                    print(f"Error al cargar el archivo {agent_file}: {e}")
                    continue
