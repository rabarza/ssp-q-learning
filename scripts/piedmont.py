...
origin_node = 53017091
target_node = 53035699
costs_distribution = "uniform"  # Se puede cambiar a "lognormal"

# Definir la ruta para guardar los resultados
graph_path = os.path.join(
    RESULTS_DIR, f"{location_name}/{origin_node}-{target_node}/"
)  # noqa: E501
save_path = os.path.join(
    RESULTS_DIR, f"{location_name}/{origin_node}-{target_node}/{costs_distribution}/"
)  # noqa: E501

# Descargar el grafo de la ciudad de Santiago y preprocesarlo
...
min_length = 1  # Longitud mínima de los arcos
edges_to_remove = [
    (u, v) for u, v, data in graph.edges(data=True) if data.get("length", 0) < min_length
]
...
assert (
    origin_node in graph.nodes and target_node in graph.nodes
), "Los nodos de origen y destino deben estar en el grafo."

# Guardar el grafo preprocesado en un archivo GraphML (opcional)
# output_graphml_path = os.path.join("santiago_graph.graphml")
# ox.save_graphml(graph, output_graphml_path)
# Guardar el grafo preprocesado en un archivo PDF (opcional)
save_graph_plot_as_pdf(graph, origin_node, target_node, graph_path, location_name)
...
shortest_path = get_shortest_path_from_policy(
    optimal_policy, origin_node, target_node
)  # noqa: E501
...
num_episodes = 20000
# formula = '250 / (t + 250)' # global clock
formula = "250 / (N(s,a) + 250)"  # local clock
